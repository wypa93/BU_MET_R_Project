...
library(plotly)
fig1<-plot_ly(data_1_no_out,x=~ConvertedCompYearly, type = "histogram",name='The population dataset')%>%
  layout(xaxis= list(showticklabels = FALSE)); fig1

fig1_bx <- plot_ly(data_1_no_out,x=~ConvertedCompYearly, type = "box",name='The population dataset')%>%
...
top_states <- sort(table(data_1_no_out$US_State),decreasing=TRUE)[1:5]
#taking the subset of these countries
states_srs <- subset(data_1_no_out,data_1_no_out$US_State %in% names(top_states))
set.seed(9999)
size = 700
#randomly chooses rows for further analysis
s<-srswor(size, nrow(states_srs))
states_srs_500 <- countries_srs[s != 0, ]
fig2<-plot_ly(states_srs_500,x=~ConvertedCompYearly, type = "histogram",name='SRS without replacement')%>%
  layout(xaxis= list(showticklabels = FALSE)); fig2
fig2_bx <- plot_ly(states_srs_500,x=~ConvertedCompYearly, type = "box",name='SRS without replacement')%>%
